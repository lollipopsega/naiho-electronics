...
if(homeGrid&&window.naihoCatalog){
  const categoryClass=p=>p.category==='Refrigeradores'?'refrigeradores':p.category==='Lavadoras'?'lavadoras':`cocinas ${p.specs[0].charAt(0)}-hornallas`;
  ...
  if(heading)heading.textContent='25 modelos para elegir';
  if(intro)intro.textContent='10 refrigeradores, 14 cocinas y una nueva lavadora NAIHO, cada producto con su ficha completa y funciones.';
}
